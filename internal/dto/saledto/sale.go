package saledto

type SaleItem struct {
	ID   int64 `json:"id" example:"1"`  // 銷售id
	Type int64 `json:"type" example:"3"`  // 銷售類型(1:免費課表/2:訂閱課表/3:付費課表)
	Name string `json:"name" example:"銅級課表"` // 銷售名稱
	Twd  float64 `json:"twd" example:"330"` // 台幣價格
	Identifier string `json:"identifier" example:"com.fitness.xxx"` // 銷售識別碼
	CreateAt string `json:"create_at" example:"2021-08-01 10:00:00"` //創建時間
	UpdateAt string `json:"update_at" example:"2021-08-01 10:00:00"` //更新時間
}

type SaleSummary struct {
	ID   int64 `json:"id" example:"1"`    // 銷售id
	Type int64  `json:"type" example:"3"`   // 銷售類型(1:免費課表/2:訂閱課表/3:付費課表)
	Name string `json:"name" example:"銅級課表"`   // 銷售名稱
	Price  float64 `json:"price" example:"330"`   // 台幣價格
}