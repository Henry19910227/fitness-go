...
type Trainer struct {
	...
	TrainerStatistic *TrainerStatistic `gorm:"foreignKey:user_id;references:user_id"` // 教練統計
}
...
type TrainerDetail struct {
	...
	CreateAt           string               `gorm:"column:create_at"`                       // 創建日期
	UpdateAt           string               `gorm:"column:update_at"`                       // 修改日期
	BankAccount        *BankAccount         `gorm:"foreignkey:user_id;references:user_id"`  // 銀行資訊
	TrainerAlbumPhotos []*TrainerAlbumPhoto `gorm:"foreignkey:user_id;references:user_id"`  // 教練相簿
	Certificates       []*Certificate       `gorm:"foreignkey:user_id;references:user_id"`  // 教練證照
	Cards              []*Card              `gorm:"foreignkey:user_id;references:user_id" ` // 教練身分證
}

func (TrainerDetail) TableName() string {
	return "trainers"
}
...
func (TrainerSummary) TableName() string {
	return "trainers"
}
