...
func (r *repository) List(input *model.ListInput) (outputs []*model.Output, amount int64, err error) {
	...
	if input.IsDeleted != nil {
		db = db.Where("food_categories.is_deleted = ?", *input.IsDeleted)
	}
	// Custom Where
	...
	if len(input.OrderField) > 0 && len(input.OrderType) > 0 {
		db = db.Order(fmt.Sprintf("food_categories.%s %s", input.OrderField, input.OrderType))
	}
	// Custom Order
	if input.Orders != nil {
