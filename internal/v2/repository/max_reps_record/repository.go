...
func (r *repository) Find(input *model.FindInput) (output *model.Output, err error) {
	...
	//加入 id 篩選條件
	if input.ID != nil {
		db = db.Where("max_reps_records.id = ?", *input.ID)
	}
	//加入 user_id 篩選條件
	if input.UserID != nil {
		db = db.Where("max_reps_records.user_id = ?", *input.UserID)
	}
	//加入 action_id 篩選條件
	if input.ActionID != nil {
		db = db.Where("max_reps_records.action_id = ?", *input.ActionID)
	}
	// Custom Where
...
func (r *repository) List(input *model.ListInput) (outputs []*model.Output, amount int64, err error) {
	...
	if len(input.OrderField) > 0 && len(input.OrderType) > 0 {
		db = db.Order(fmt.Sprintf("max_reps_records.%s %s", input.OrderField, input.OrderType))
	}
	// Custom Order
	if input.Orders != nil {
