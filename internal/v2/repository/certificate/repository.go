...
func (r *repository) Deletes(inputs []*model.DeleteInput) (err error) {
	ids := make([]int64, 0)
	...
	err = r.db.Where("id IN (?)", ids).Delete(&model.Table{}).Error
	return err
}
...
func (r *repository) List(input *model.ListInput) (outputs []*model.Output, amount int64, err error) {
	...
	// id 篩選條件
	if input.ID != nil {
		db = db.Where("certificates.id = ?", *input.ID)
	}
	// Custom Where
	...
	if len(input.OrderField) > 0 && len(input.OrderType) > 0 {
		db = db.Order(fmt.Sprintf("certificates.%s %s", input.OrderField, input.OrderType))
	}
	// Custom Order
	if input.Orders != nil {
