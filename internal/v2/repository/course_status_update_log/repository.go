...
func (r *repository) List(input *model.ListInput) (outputs []*model.Output, amount int64, err error) {
	...
	if input.CourseID != nil {
		db = db.Where("course_status_update_logs.course_id = ?", *input.CourseID)
	}
	// course_status 篩選條件
	if input.CourseStatus != nil {
		db = db.Where("course_status_update_logs.course_status = ?", *input.CourseStatus)
	}
	// Custom Where
	...
	db = db.Count(&amount)
	// Select
	db = db.Select("course_status_update_logs.*")
	// Paging
	...
	if len(input.OrderField) > 0 && len(input.OrderType) > 0 {
		db = db.Order(fmt.Sprintf("course_status_update_logs.%s %s", input.OrderField, input.OrderType))
	}
	// Custom Order
	if input.Orders != nil {
