...
func (r *repository) List(input *model.ListInput) (outputs []*model.Output, amount int64, err error) {
	...
	//加入 year 篩選條件
	if input.Year != nil {
		db = db.Where("user_unsubscribe_monthly_statistics.year = ?", *input.Year)
	}
	//加入 month 篩選條件
	if input.Month != nil {
		db = db.Where("user_unsubscribe_monthly_statistics.month = ?", *input.Month)
	}
	// Custom Where
	...
	db = db.Count(&amount)
	// Select
	db = db.Select("user_unsubscribe_monthly_statistics.*")
	// Paging
	...
	if len(input.OrderField) > 0 && len(input.OrderType) > 0 {
		db = db.Order(fmt.Sprintf("user_unsubscribe_monthly_statistics.%s %s", input.OrderField, input.OrderType))
	}
	// Custom Order
	if input.Orders != nil {
...
// Statistic SQL
/*
INSERT INTO user_unsubscribe_monthly_statistics (year, month, total, male, female, age_13_17, age_18_24, age_25_34, age_35_44, age_45_54, age_55_64, age_65_up)
SELECT
  2022 AS year,
  6 AS month,
...
INNER JOIN receipts ON orders.id = receipts.order_id
INNER JOIN subscribe_logs ON receipts.transaction_id = subscribe_logs.transaction_id
WHERE YEAR(subscribe_logs.create_at) = 2022 AND MONTH(subscribe_logs.create_at) = 6 AND subscribe_logs.type = 'expired'
...
func (r *repository) Statistic(input *model.StatisticInput) (err error) {
	err = r.db.Exec("INSERT INTO user_unsubscribe_monthly_statistics (year, month, total, male, female, age_13_17, age_18_24, age_25_34, age_35_44, age_45_54, age_55_64, age_65_up) "+
		"SELECT "+
		"? AS year, "+
		"? AS month, "+
		...
		"INNER JOIN receipts ON orders.id = receipts.order_id "+
		"INNER JOIN subscribe_logs ON receipts.transaction_id = subscribe_logs.transaction_id "+
		"WHERE YEAR(subscribe_logs.create_at) = ? AND MONTH(subscribe_logs.create_at) = ? AND subscribe_logs.type = 'expired' "+
		"ON DUPLICATE KEY UPDATE "+
		"total = VALUES(total), "+
		"male = VALUES(male), "+
		"female = VALUES(female), "+
