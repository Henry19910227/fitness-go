package optional

type IDField struct {
	ID *int64 `json:"id,omitempty" uri:"review_image_id" gorm:"column:id" binding:"omitempty" example:"1"` //圖片id
}
type ReviewIDField struct {
	ReviewID *int64 `json:"review_id,omitempty" gorm:"column:review_id" binding:"omitempty" example:"1"` //評論id
}
type ImageField struct {
	Image *string `json:"image,omitempty" gorm:"column:image" binding:"omitempty" example:"1234.jpg"` //圖片
}
type CreateAtField struct {
	CreateAt *string `json:"create_at,omitempty" gorm:"column:create_at" binding:"omitempty" example:"2022-06-14 00:00:00"` //創建時間
}
