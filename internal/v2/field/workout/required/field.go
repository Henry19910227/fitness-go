...
type IDField struct {
	ID int64 `json:"id" gorm:"column:id" example:"1"` // 訓練 id
}
type PlanIDField struct {
	PlanID int64 `json:"plan_id" gorm:"column:plan_id" example:"1"` // 計畫 id
}
...
type EquipmentField struct {
	Equipment string `json:"equipment" form:"equipment" gorm:"column:equipment" binding:"omitempty,min=0,max=10" example:"2,3,6"` // 所需器材(1:無需任何器材/2:啞鈴/3:槓鈴/4:固定式器材/5:彈力繩/6:壺鈴/7:訓練椅/8:瑜珈墊/9:其他)
}
