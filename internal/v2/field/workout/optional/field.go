...
type EquipmentField struct {
	Equipment *string `json:"equipment,omitempty" form:"equipment" gorm:"column:equipment" binding:"omitempty,equipment,min=0,max=10" example:"2,3,6"` // 所需器材(1:無需任何器材/2:啞鈴/3:槓鈴/4:固定式器材/5:彈力繩/6:壺鈴/7:訓練椅/8:瑜珈墊/9:其他)
}
...
type EndAudioField struct {
	EndAudio *string `json:"end_audio,omitempty" gorm:"column:end_audio" example:"123.mp3"` // 結束語音
}
type WorkoutSetCountField struct {
	WorkoutSetCount *int `json:"workout_set_count,omitempty" gorm:"column:workout_set_count" example:"10"` // 動作組數
}
