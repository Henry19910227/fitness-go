...
func (r *resolver) APIUpdateTrainerWorkoutSetOrders(tx *gorm.DB, input *model.APIUpdateTrainerWorkoutSetOrdersInput) (output model.APIUpdateTrainerWorkoutSetOrdersOutput) {
	defer tx.Rollback()
	// 查詢關聯課表
	findCourseInput := courseModel.FindInput{}
	findCourseInput.WorkoutID = util.PointerInt64(input.Uri.WorkoutID)
