...
// APIUpdateTrainerWorkoutSetOrdersInput /v2/trainer/workout/{workout_id}/workout_set_orders [PUT] 修改訓練組的順序
type APIUpdateTrainerWorkoutSetOrdersInput struct {
	UserID int64 `json:"user_id" binding:"required" example:"10001"` // 用戶 id
	Uri    APIUpdateTrainerWorkoutSetOrderUri
	Body   APIUpdateTrainerWorkoutSetOrderBody
}
type APIUpdateTrainerWorkoutSetOrderUri struct {
	WorkoutIDRequired
}
type APIUpdateTrainerWorkoutSetOrderBody struct {
	WorkoutSetOrders []struct {
		WorkoutSetIDRequired
		SeqRequired
	} `json:"workout_set_orders" binding:"required"` //訓練組排序
