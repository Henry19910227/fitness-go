...
type CategoryOptional struct {
	Category *int `json:"category,omitempty" form:"category" binding:"omitempty,oneof=1 2 3 4 5" example:"1234.mp4"` //分類(1:重量訓練/2:有氧/3:HIIT/4:徒手訓練/5:其他)
}
type BodyOptional struct {
	Body *int `json:"body,omitempty" form:"body" binding:"omitempty,oneof=1 2 3 4 5 6 7 8" example:"8"` //身體部位(1:全身/2:核心/3:手臂/4:背部/5:臀部/6:腿部/7:肩膀/8:胸部)
}
type EquipmentOptional struct {
	Equipment *int `json:"equipment,omitempty" form:"equipment" binding:"omitempty,oneof=1 2 3 4 5 6 7 8 9" example:"1"` //器材(1:無需任何器材/2:啞鈴/3:槓鈴/4:固定式器材/5:彈力繩/6:壺鈴/7:訓練椅/8:瑜珈墊/9:其他)
}
