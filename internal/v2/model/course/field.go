...
type IDField struct {
	ID *int64 `json:"id,omitempty" uri:"course_id" form:"id" gorm:"column:id" binding:"omitempty" example:"2"` // 課表 id
}
type UserIDField struct {
	UserID *int64 `json:"user_id,omitempty" gorm:"column:user_id" example:"10001"` // 用戶 id
}
type SaleTypeField struct {
	SaleType *int `json:"sale_type,omitempty" form:"sale_type" gorm:"column:sale_type" binding:"omitempty,oneof=1 2 3" example:"3"` // 銷售類型(1:免費課表/2:訂閱課表/3:付費課表)
}
...
type IntroField struct {
	Intro *string `json:"intro,omitempty" gorm:"column:intro" example:"增肌專用課表"` // 課表介紹
}
type FoodField struct {
	Food *string `json:"food,omitempty" gorm:"column:food" example:"多吃雞胸肉"` // 飲食建議
}
type LevelField struct {
	Level *int `json:"level,omitempty" gorm:"column:level" example:"4"` // 強度(1:初級/2:中級/3:中高級/4:高級)
}
