...
// @Success 200 {object} api_get_cms_trainers.Output "成功!"
// @Failure 400 {object} base.Output "失敗!"
// @Router /v2/cms/trainers [GET]
func (c *controller) GetCMSTrainers(ctx *gin.Context) {
	var input api_get_cms_trainers.Input
	if err := ctx.ShouldBindQuery(&input.Query); err != nil {
		ctx.JSON(http.StatusBadRequest, baseModel.BadRequest(util.PointerString(err.Error())))
		return
	}
	output := c.resolver.APIGetCMSTrainers(&input)
	ctx.JSON(http.StatusOK, output)
}

// GetCMSTrainer 獲取教練詳細
// @Summary 獲取教練詳細
// @Description 獲取教練詳細
// @Tags CMS會員管理_v2
// @Accept json
...
// @Success 200 {object} api_get_cms_trainer.Output "成功!"
// @Failure 400 {object} base.Output "失敗!"
// @Router /v2/cms/trainer/{user_id} [GET]
func (c *controller) GetCMSTrainer(ctx *gin.Context) {
	var input api_get_cms_trainer.Input
	if err := ctx.ShouldBindUri(&input.Uri); err != nil {
		ctx.JSON(http.StatusBadRequest, baseModel.BadRequest(util.PointerString(err.Error())))
		return
	}
	output := c.resolver.APIGetCMSTrainer(&input)
	ctx.JSON(http.StatusOK, output)
}
