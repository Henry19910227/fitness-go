...
// @Param json_body body api_calculate_rda.Body true "輸入參數"
// @Success 200 {object} api_calculate_rda.Output "成功!"
// @Failure 400 {object} base.Output "失敗!"
// @Router /v2/calculate_rda [POST]
func (c *controller) CalculateRDA(ctx *gin.Context) {
	input := api_calculate_rda.Input{}
	input.UserID = ctx.MustGet("uid").(int64)
	if err := ctx.ShouldBindJSON(&input.Body); err != nil {
		ctx.JSON(http.StatusBadRequest, baseModel.BadRequest(util.PointerString(err.Error())))
		return
	}
	output := c.resolver.APICalculateRDA(&input)
