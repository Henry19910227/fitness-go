...
// @Security fitness_token
// @Param plan_id path int64 true "計畫id"
// @Param json_body body api_update_trainer_plan.Body true "輸入參數"
// @Success 200 {object} api_update_trainer_plan.Output "0:Success/ 9000:Bad Request/ 9005:Invalid Token/ 9006:Permission denied"
// @Failure 400 {object} base.Output "失敗!"
// @Router /v2/trainer/plan/{plan_id} [PATCH]
func (c *controller) UpdateTrainerPlan(ctx *gin.Context) {
	input := api_update_trainer_plan.Input{}
	input.UserID = ctx.MustGet("uid").(int64)
	if err := ctx.ShouldBindUri(&input.Uri); err != nil {
		ctx.JSON(http.StatusBadRequest, baseModel.BadRequest(util.PointerString(err.Error())))
		return
	...
	output := c.resolver.APIUpdateTrainerPlan(&input)
	ctx.JSON(http.StatusOK, output)
}
