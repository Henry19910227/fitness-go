...
import (
	...
	"github.com/Henry19910227/fitness-go/internal/v2/resolver/user_promote_monthly_statistic"
	"github.com/gin-gonic/gin"
	"net/http"
)

type controller struct {
	resolver user_promote_monthly_statistic.Resolver
}

func New(resolver user_promote_monthly_statistic.Resolver) Controller {
	return &controller{resolver: resolver}
}

// GetCMSUserPromoteMonthlyStatistic 獲取用戶晉升教練統計月報表
...
func (c *controller) GetCMSUserPromoteMonthlyStatistic(ctx *gin.Context) {
	var input api_get_cms_statistic_monthly_user_promote.Input
	if err := ctx.ShouldBindQuery(&input.Query); err != nil {
		ctx.JSON(http.StatusBadRequest, baseModel.BadRequest(util.PointerString(err.Error())))
		return
	}
	output := c.resolver.APIGetCMSUserPromoteStatistic(&input)
