...
type CreateActionBody struct {
	...
	Equipment int `json:"equipment" binding:"required,oneof=1 2 3 4 5 6 7 8" example:"1"` //器材(1:無需任何器材/2:啞鈴/3:槓鈴/4:固定式器材/5:彈力繩/6:壺鈴/7:訓練椅/8:其他)
...
type UpdateActionBody struct {
	...
	Equipment *int `json:"equipment" binding:"omitempty,oneof=1 2 3 4 5 6 7 8" example:"1"` //器材(1:無需任何器材/2:啞鈴/3:槓鈴/4:固定式器材/5:彈力繩/6:壺鈴/7:訓練椅/8:其他)
...
type SearchActionsQuery struct {
	...
	Equipment *string `form:"equipment" binding:"omitempty,action_equipment" example:"1,3,5"` //器材(1:無需任何器材/2:啞鈴/3:槓鈴/4:固定式器材/5:彈力繩/6:壺鈴/7:訓練椅/8:其他)
}
...
var ActionBody validator.Func = func(fl validator.FieldLevel) bool {
	return validateActionFieldByRange(fl, 1, 8)
}
