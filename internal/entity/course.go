...
type CourseTemplate struct {
	...
	CreateAt     string            `gorm:"column:create_at"`                      // 創建時間
	UpdateAt     string            `gorm:"column:update_at"`                      // 更新時間
	User         *UserTemplate     `gorm:"foreignKey:id;references:user_id"`      // 課表創建者
	Trainer      *TrainerTemplate  `gorm:"foreignKey:user_id;references:user_id"` // 課表創建者
	SaleItem     *SaleItemTemplate `gorm:"foreignkey:id;references:sale_id"`      // 銷售項目
}
