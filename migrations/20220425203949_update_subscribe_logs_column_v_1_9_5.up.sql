...
SET @updateOriginalTransactionIDColumn = (SELECT IF(
  (
    ...
    CONCAT("ALTER TABLE ", @tablename, " MODIFY COLUMN `original_transaction_id` varchar(50) NOT NULL DEFAULT '' COMMENT '初始交易 id'"),
    "SELECT 1"
));
...
SET @updateTransactionIDColumn = (SELECT IF(
  (
    ...
    CONCAT("ALTER TABLE ", @tablename, " MODIFY COLUMN `transaction_id` varchar(50) NOT NULL DEFAULT '' COMMENT '交易 id'"),
    "SELECT 1"
));
